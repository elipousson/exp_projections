...
make_pyramid_chart <- function(df, type) {
  df %>%
    ...
    scale_y_continuous(
      ...
      labels = c(
        str_wrap("Projected lower", width = 10), "-80%", "-60%", "-40%", "-20%", "0%",
...
output %>%
  mutate(
    `Percent Change` = scales::percent(`Percent Change`, accuracy = 0.01),
    ...
    `Percent Change` = cell_spec(`Percent Change`, "html",
      color = "white", bold = TRUE,
      background =
        case_when(
          abs(output$`Percent Change`) < .04 ~ "#69B34C",
...
output %>%
  mutate(
    ...
    `Agency Name` = cell_spec(`Agency Name`, "html",
      color = "white", bold = TRUE,
      background =
        case_when(
          abs(output$`Percent Change`) < .04 ~ "#69B34C",
