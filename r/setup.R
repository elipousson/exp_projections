...
setup_internal <- function(proj = "quarterly") {
  ...
  l <- list(
    ...
    (
      paste0(
        ...
        "/Projections Year/2. Monthly Expenditure Data"
      ), "^[^~]*Expenditure",
      recursive = TRUE
    ),
  ...
  l$seasonal <- switch(params$qt,
    "1" = paste0(
      "([YTD Exp] + AVERAGE([July],[August],[September])*(12-",
      l$months.in, "-1))*.5 + AVERAGE([July],[August],[September])*1.5"
    ),
    "2" = paste0(
      "([YTD Exp] + AVERAGE([October]-[December])*(12-",
      l$months.in, "-1))*.5 + AVERAGE([July]:[September])*1.5"
    ),
    "3" = paste0(
      "([YTD Exp] + AVERAGE([October]-[March])*(12-",
      l$months.in, "-1))*.5 + AVERAGE([July]:[September])*1.5"
    )
  )
