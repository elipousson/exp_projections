  ...
  tryCatch(
    {
      ...
      agencies$output.file <- paste0(
        "G:/Agencies/", agencies$agency, "/File Distribution/",
      ...
      if (nrow(agencies$position) > 0) {
        ...
        highlight_bpfs_cols <-
          c(
            which(grepl("^Position ID|FY22 Adopted Status", names(agencies$position))),
        ...
        highlight_workday_cols <-
          c(
            which(grepl("Status 20|Employee 20", names(agencies$position))),
        ...
        excel <- export_excel(
          ...
          addStyle("Positions", highlight_bpfs,
            rows = 1,
            cols = highlight_bpfs_cols
          ) %T>%
          addStyle("Positions", highlight_workday,
            rows = 1,
            cols = highlight_workday_cols
          ) %T>%
...
add_dgs_oso_tab <- function(expend, path) {
  ...
  output_file <- paste0(
    "G:/Agencies/General Services/File Distribution/",
...
make_arpa_fund_file <- function(expend, path) {
  ...
  output_file <- paste0(
    "G:/Agencies/MR American Rescue Plan Act/File Distribution/",
    params$file_name, ".xlsx"
  )
...
separate_finance_files <- function(agencies, path) {
  ...
  for (i in unique(agencies$expend$Bureau)) {
    finance_file <- paste0(
      "G:/Agencies/Finance/File Distribution/",
