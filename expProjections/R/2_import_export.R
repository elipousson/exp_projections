...
export_analyst_calcs_workday <- function(df) {
  ...
  if (nrow(check) > 0) {
    ...
    warning("No duplicate caculations found.")
  }
...
run_summary_reports <- function(df) {
  ...
  reports <- list(
    object = df %>%
      group_by(
        `Agency ID`, `Agency Name`, `Service ID`, `Service Name`,
