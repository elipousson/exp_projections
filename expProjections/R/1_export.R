  ...
  tryCatch(
    {
      style <- list(
        cell.bg = createStyle(
          fgFill = "lightcyan", border = "TopBottomLeftRight",
      ...
      dataValidation(
        excel, 1,
        rows = style$rows, type = "list", value = "Calcs!$A$2:$A$10",
      ...
      conditionalFormatting(
        ...
        cols = grep(paste0(c(cols$proj, cols$sur_def),
          collapse = "|"
        ), names(data$line.item))
      )
      addStyle(excel, 1, style$cell.bg,
        rows = 1,
      ...
      addStyle(excel, 1, style$formula.num,
        rows = style$rows,
        gridExpand = TRUE, stack = FALSE,
        cols = grep(paste0(c(cols$proj, cols$sur_def),
          collapse = "|"
      ...
      writeComment(
        excel, 1, grep(paste0("^", cols$calc), names(data$line.item)),
        row = 1,
        createComment(
          paste(
            "Please use the drop-down instead of typing. If 'Manual' is selected, you should overwrite the formula in the",
      ...
      writeComment(
        excel, 1, grep(paste0("^Q", params$qt, " Manual Formula"), names(data$line.item)),
        row = 1,
        createComment(
  ...
  tryCatch(
    {
      ...
      export_formatted_pivot <- function(type) {
        ...
        excel <- suppressMessages(export_excel(data[[tolower(type)]],
          paste0("Pivot-", type), data$file, "existing",
          table_name = type,
          col_width = c(rep("auto", 4), rep(15, ncol(data[[tolower(type)]]) - 4)),
          save = FALSE
        ))
        addStyle(
          excel, paste0("Pivot-", type), style$cell.bg,
          rows = 1,
          gridExpand = TRUE, stack = FALSE,
          cols = grep(paste0(c(cols$proj, cols$sur_def),
            collapse = "|"
        ...
        addStyle( # total row
          excel, paste0("Pivot-", type), style$total,
          rows = max(style$rows),
          gridExpand = TRUE, stack = FALSE,
          cols = 1:ncol(data$object)
        )
        conditionalFormatting(
          excel, paste0("Pivot-", type),
          rows = style$rows, style = style$negative,
          type = "expression", rule = "<0",
          cols = grep(paste0(c(cols$proj, cols$sur_def),
            collapse = "|"
        ...
        addStyle(excel, paste0("Pivot-", type), style$formula.num,
          cols = style$cols,
      ...
      conditionalFormatting(
        excel, "Pivot-Service SurDef",
        cols = style$cols, rows = style$rows,
