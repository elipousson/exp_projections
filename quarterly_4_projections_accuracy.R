...
prep_data <- function(df, type) {
  ...
  df <- df %>%
    ...
    bind_total_row(
      total_col = "Quarter", total_name = "Avg.",
      group_col = c("FY Total Budget", "FY Actual", group)
    ) %>%
    mutate(
      Projection = ifelse(Quarter == "Avg.", Projection / params$qtr, Projection),
  ...
  assert_that(
    check$`FY Total Budget` == totals$`FY Total Budget`,
...
make_pyramid_chart <- function(df, type) {
  df %>%
    ...
    scale_y_continuous(
      ...
      labels = c(
        str_wrap("Projected lower", width = 10), "-20%", "-15%", "-10%", "-5%", "0%",
...
objects <- projections %>%
  ...
  mutate(
    `Pct Budget Spent` = `FY Actual` / `FY Total Budget`,
...
objects_output <- select(objects, c(
  Object, `FY Total Budget`, `FY Actual`, `Pct Budget Spent`,
...
agencies_output <- select(agencies, c(
  `Agency Name`, `FY Total Budget`, `FY Actual`, `Pct Budget Spent`,
  `Q1 Projection`, `Q1 Difference`, `Q1 Percent Change`,
  `Q2 Projection`, `Q2 Difference`, `Q2 Percent Change`,
  `Q3 Projection`, `Q3 Difference`, `Q3 Percent Change`
))
