...
conn <- AzureGraph::create_graph_login(
  tenant = "bmore",
...
if (dim(files)[1] != 53) {
  warning("Check files for missing or extra.")
} else {
  warning("53 agency files found.")
}
...
if (is.na(params$compiled_edit)) {
  data <- list.files(internal$analyst_files,
    pattern = paste0("FY", params$fy, " .*Q", params$qtr, "-.*xlsx"),
  ...
  df <- compiled %>%
    ...
    group_by(
      Agency, Service, `Cost Center`, Fund, Grant,
  ...
  df <- df %>%
    ...
    select(
      `Agency`:`Spend Category`, !!paste0("FY", params$fy - 1, " Actuals"), `YTD Actuals`, !!paste0("FY", params$fy, " Budget"),
...
missing <- list.zip(
  agencies = unique(analysts$`Agency`)[!unique(analysts$`Agency`) %in% compiled$Agency],
